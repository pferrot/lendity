package com.pferrot.lendity.jobs.tolendemailreminder;
...
public class ToLendEmailReminderJob extends TransactionalQuartzJobBean {
	
	private final static Log log = LogFactory.getLog(ToLendEmailReminderJob.class);
	...
	protected void executeTransactional(final JobExecutionContext pJobExecutionContext)
		...
		if (log.isWarnEnabled()) {
			log.warn("Executing ToLendEmailReminderJob");
		}
		try {
			...
			if (log.isErrorEnabled()) {
				log.error("Exception executing ToLendEmailReminderJob", e);
			}
			throw new JobExecutionException(e);
		}
		finally {
			if (log.isWarnEnabled()) {
				log.warn("Done executing ToLendEmailReminderJob");
			}
		}
	}
	...
	private void sendEmailForAllTransactions() {
		final List<LendTransaction> transactions = lendTransactionService.findLendTransactionsToLendSoon();
	...
	private void sendForOneTransaction(final LendTransaction pLendTransaction) {
		...
		if (pLendTransaction.getLender() != null &&
			...
			// TODO: localization
			final String velocityTemplateLocation = "com/pferrot/lendity/emailtemplate/reminder/tolend/lender/fr";
			...
			mailManager.send(Configuration.getNoReplySenderName(), 
					         ...
					         null, 
					         null,
					         Configuration.getSiteName() + ": rappel, objet à prêter",
					         objects, 
		...
		if (pLendTransaction.getBorrower() != null &&
			pLendTransaction.getBorrower().isEnabled()) {
				...
				// TODO: localization
				final String velocityTemplateLocation = "com/pferrot/lendity/emailtemplate/reminder/tolend/borrower/fr";
				...
				mailManager.send(Configuration.getNoReplySenderName(), 
						         ...
						         null, 
						         null,
						         Configuration.getSiteName() + ": rappel, un objet doit t'être prêté",
						         objects, 
						         velocityTemplateLocation,
						         inlineResources);
			}
